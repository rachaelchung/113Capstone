...
There's no build step — edit the files directly and hit **Reload** on the extension card at `chrome://extensions`.

Quick iteration loop:

1. Edit a file (e.g. `popup.js`).
2. Go to `chrome://extensions`.
3. Click the reload (↻) icon on the **head empty** card.
4. Re-open the popup or refresh a page to see the change.

To debug the service worker, click **service worker** on the extension card to open DevTools. For the popup, right-click inside the popup → **Inspect**. For in-page overlays, use the normal page DevTools console.
...
- `storage` — to save your off-limits site list, cached session, and preferences
- `tabs` — to detect when you navigate to an off-limits site
- `host_permissions` for the API origins (`127.0.0.1:8000`, `127.0.0.1:8001`, `localhost:8000`, `localhost:8001`, `*.onrender.com`) so the extension can authenticate and sync

It runs a content script on all URLs to render the overlay, and a narrower auth-bridge script on `localhost`, `127.0.0.1`, and `*.github.io` to copy your web-app session.
